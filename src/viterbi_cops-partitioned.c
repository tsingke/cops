...
int p7_ViterbiCOPSw_partitioned(DATA_COPS16* dcops, float* opt_res)
{
	int L = dcops->L;
...
int p7_ViterbiCOPSw_run(DATA_COPS16* dcops, SEQ **seqsdb, float* results)
{
	...
	p7_ViterbiCOPSw_partitioned(dcops, results);
	return maxL;
}
...
int main(int argc, char **argv)
{
	...
	{	// Use Sequence file
   	...
   	printf("Viterbi COPS Word Partitioned, model %s. ModelLen: %d, #Segms: %d, SeqL.: %d, #seqs: %d, Partition: %d, #parts: %d\n",
			hmmfile, gm1->M, (int) ceil(gm1->M/SSE16_NVALS), L, SSE16_NVALS*N*NROUNDS, dcops->partition, dcops->Npartitions);
