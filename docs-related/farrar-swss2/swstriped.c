    ...
    for (i = 0; i < ALPHA_SIZE; ++i) {
        ...
        for (j = 0; j < segSize; ++j) {
            for (k = j; k < nCount; k += segSize) {
                ...
                *pc++ = (char) (weight - bias);	// bias is < 0; here we add bias
            }
        }
    }
              ...
              unsigned short   bias)
{
    ...
    for (i = 0; i < dbLength; ++i)
    {
        ...
        for (j = 0; j < iter; j++)
        {
            ...
            /* add score to vH */
            vH = _mm_adds_epu8 (vH, *(pvScore + j));	// the profile is biased
            vH = _mm_subs_epu8 (vH, vBias);	// unbiass
